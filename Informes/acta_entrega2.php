...
$sql = "select p.PROD_COD, p.PROD_NOMBRE, c.CATPROD_NOMBRE, sd.SPRODR_CANT from PRODUCTOS p
join CATEGORIA_PRODUCTO c on p.catprod_id = c.catprod_id join SPROD_DETALLE sd on sd.prod_cod = p.prod_cod
where sd.sprod_id = $sprod_id /*AND sd.SPRODR_ESTADO = 'ACEPTADA'*/";	
...
$pdf->SetWidths(array(10,25,80,50,25));
//max195
$pdf->Row(array("N°", "COD. MATERIAL", "DETALLE MATERIAL", "CATEGORIA", "CANTIDAD"));
...
while( $rows = mysqli_fetch_assoc($resultset) ) {
	$pdf->Row(array($k,$rows[PROD_COD],utf8_decode($rows[PROD_NOMBRE]),utf8_decode($rows[CATPROD_NOMBRE]),$rows[SPRODR_CANT]));
	$k++;
}
...
while( $rows2 = mysqli_fetch_assoc($resultset) ) {
	$pdf->Row(array($k,$rows2[PROD_COD],utf8_decode($rows2[PROD_NOMBRE]),utf8_decode($rows2[CATPROD_NOMBRE]),$rows2[SPRODR_CANT]));
	$k++;
}
