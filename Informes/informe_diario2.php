...
---------
59	20170710	NULL
123	20170710	20064.00
59	20170711	90928.00
123	20170711	34760.00
123	20170712	36256.00
59	20170712	73568.00
59	20170713	71253.00
123	20170713	42592.00
59	20170714	53240.00
123	20170714	16280.00
...
//123	MINI	1066557.00
		...
		JOIN PLANIFICACION_OPER AS PLANO ON PLANO.PLAN_DIA = PLAN.PLAN_DIA AND PLANO.ACTIV_ID = PLAN.ACTIV_ID
		JOIN ACTIVIDAD AS ACT ON ACT.ACTIV_ID = PLAN.ACTIV_ID
		JOIN METAS_INF AS M ON M.PER_AGNO = S.PER_AGNO AND M.PER_MES=S.PER_MES
		where S.PER_AGNO = $agno and S.PER_MES = '$mes' AND PLAN.ACTIV_ID = 123
		";
		$resultado2 = mysqli_query($link, $sql) or die("database error:". mysqli_error($link));
		$fila = mysqli_fetch_row($resultado2);
		$cant_123 = $fila[0];
		...
		if($cant_123 > 0 and $numero_setos > 0){
			...
			JOIN PLANIFICACION_OPER AS PLANO ON PLANO.PLAN_DIA = PLAN.PLAN_DIA AND PLANO.ACTIV_ID = PLAN.ACTIV_ID
			JOIN ACTIVIDAD AS ACT ON ACT.ACTIV_ID = PLAN.ACTIV_ID
			JOIN SEMANAS AS S ON S.PER_AGNO = PLAN.PER_AGNO and S.SEM_NUM = PLAN.SEM_NUM 
			WHERE S.PER_MES = '$mes' AND PLAN.PER_AGNO = $agno AND PLAN.ACTIV_ID = 123 AND PLANO.PLANO_CANT > 0";			
			$resultado2 = mysqli_query($link, $sql) or die("database error:". mysqli_error($link));
			$fila = mysqli_fetch_row($resultado2);
			$count_123 = $fila[0];		
...
//59	PINO	
		...
		JOIN PLANIFICACION_OPER AS PLANO ON PLANO.PLAN_DIA = PLAN.PLAN_DIA AND PLANO.ACTIV_ID = PLAN.ACTIV_ID
		JOIN ACTIVIDAD AS ACT ON ACT.ACTIV_ID = PLAN.ACTIV_ID
		where S.PER_AGNO = $agno and S.PER_MES = '$mes' AND PLAN.ACTIV_ID = 59
		";
		$resultado2 = mysqli_query($link, $sql) or die("database error:". mysqli_error($link));
		$fila = mysqli_fetch_row($resultado2);
		$cant_59 = $fila[0];
		...
		if($cant_59 > 0){
			...
			JOIN ACTIVIDAD AS ACT ON ACT.ACTIV_ID = PLAN.ACTIV_ID
			JOIN SEMANAS AS S ON S.PER_AGNO = PLAN.PER_AGNO and S.SEM_NUM = PLAN.SEM_NUM 
			WHERE S.PER_MES = '$mes' AND PLAN.PER_AGNO = $agno AND PLAN.ACTIV_ID = 59 AND PLANO.PLANO_CANT > 0";			
			$resultado2 = mysqli_query($link, $sql) or die("database error:". mysqli_error($link));
			$fila = mysqli_fetch_row($resultado2);
			$count_59 = $fila[0];		
			
			$meta = $meta_mini * $count_59;			
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(8,$row_current,$meta);			
			$cumplimiento =$cant_59/$meta;			
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(9,$row_current,$cumplimiento);
...
//135	MACRO	
		...
		JOIN PLANIFICACION_OPER AS PLANO ON PLANO.PLAN_DIA = PLAN.PLAN_DIA AND PLANO.ACTIV_ID = PLAN.ACTIV_ID
		JOIN ACTIVIDAD AS ACT ON ACT.ACTIV_ID = PLAN.ACTIV_ID
		where S.PER_AGNO = $agno and S.PER_MES = '$mes' AND PLAN.ACTIV_ID = 135
		";
		$resultado2 = mysqli_query($link, $sql) or die("database error:". mysqli_error($link));
		$fila = mysqli_fetch_row($resultado2);
		$cant_135 = $fila[0];
		...
		if($cant_135 > 0){
			...
			JOIN ACTIVIDAD AS ACT ON ACT.ACTIV_ID = PLAN.ACTIV_ID
			JOIN SEMANAS AS S ON S.PER_AGNO = PLAN.PER_AGNO and S.SEM_NUM = PLAN.SEM_NUM 
			WHERE S.PER_MES = '$mes' AND PLAN.PER_AGNO = $agno AND PLAN.ACTIV_ID = 135 AND PLANO.PLANO_CANT > 0";			
			$resultado2 = mysqli_query($link, $sql) or die("database error:". mysqli_error($link));
			$fila = mysqli_fetch_row($resultado2);
			$count_135 = $fila[0];		
			
			$meta = $meta_mini * $count_135;			
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(13,$row_current,$meta);			
			$cumplimiento =$cant_135/$meta;			
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(14,$row_current,$cumplimiento);
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(++$k,$row_current,'=SUM(D'.$sum_ini.':D'.$sum_fin.')');
	$objPHPExcel->getActiveSheet(0)->getStyle('C'.$sum_ini.':E'.$sum_fin)->applyFromArray($style6);
	$objPHPExcel->getActiveSheet(0)->getStyle('B'.$row_current.':E'.$row_current)->applyFromArray($style8);		
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(++$k,$row_current,'=SUM(I'.$sum_ini.':I'.$sum_fin.')');
	$objPHPExcel->getActiveSheet(0)->getStyle('H'.$sum_ini.':J'.$sum_fin)->applyFromArray($style6);
	$objPHPExcel->getActiveSheet(0)->getStyle('G'.$row_current.':J'.$row_current)->applyFromArray($style8);	
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(++$k,$row_current,'=SUM(N'.$sum_ini.':N'.$sum_fin.')');
	$objPHPExcel->getActiveSheet(0)->getStyle('M'.$sum_ini.':O'.$sum_fin)->applyFromArray($style6);
	$objPHPExcel->getActiveSheet(0)->getStyle('L'.$row_current.':O'.$row_current)->applyFromArray($style8);	
