...
$sql = "SELECT ACT.ACTIV_ID,PLAN.PLANM_DIA,
		...
		IFNULL(ACP.PACTIV_NOMBRE,'') AS PACTIV_NOMBRE, IFNULL(ACT2.TACTIV_NOMBRE,'') AS TACTIV_NOMBRE			
			...
			LEFT JOIN ACTIVIDAD_CLASIFICACION AS ACC ON ACC.ACTIV_ID = ACT.ACTIV_ID
			LEFT JOIN ACTIVIDAD_TIPO AS ACT2 ON ACT2.TACTIV_ID = ACC.TACTIV_ID
...
/*
				$sql = "SELECT IFNULL(SUM(PLANO.PLANO_HORAS),0) * IFNULL(MET.MET_UNDXHR,0) AS PROD_ESPERADA, 
				IFNULL(SUM(PLANO.PLANO_HORAS),0) AS HORAS_PLANIF, ACT.ACTIV_NOMBRE AS ACTIVIDAD, IFNULL(MET.MET_UNDXHR,0) AS META,
				IFNULL(SUM(PLANO.PLANO_HORASPROD),0) AS HORASPROD, SUM(CASE WHEN PLANO.PLANO_ASIST = 'S' THEN 1 ELSE 0 END) AS OPERADORES,
				IFNULL(SUM(PLANO.PLANO_HORASPROD),0) * IFNULL(PLANO.PLANO_CANT,0) AS PROD_REAL
				...
				WHERE PLANO.PLAN_DIA = '$rows[PLAN_DIA]' AND PLANO.ACTIV_ID = 240";
*/
...
*/				
	...
	/*
	echo '<pre>';
	print_r($prodreal_cosecha_globulus);
	echo '</pre>';		*/			
				...
				//--$prodreal_cosecha_globulus = str_replace('.',',',$prodreal_cosecha_globulus);
	/*echo '<pre>';
	print_r($prodreal_cosecha_globulus);
	echo '</pre>';	*/			
				$hrs_cosecha_globulus = str_replace('.00','',$hrs_cosecha_globulus);
				//--$hrs_cosecha_globulus = str_replace('.',',',$hrs_cosecha_globulus);	
				
				//IFNULL(SUM(PLANO.PLANO_HORAS),0) * IFNULL(MET.MET_UNDXHR,0) AS PROD_ESPERADA,
				//SELECT IFNULL(SUM(PLANO.PLANO_HORASPROD),0) * IFNULL(MET.MET_UNDXHR,0) AS PROD_ESPERADA,
				$sql = "SELECT IFNULL(SUM(PLANO.PLANO_CANT),0) AS PROD_ESPERADA, 
				IFNULL(SUM(PLANO.PLANO_HORASPROD),0) AS HORAS_PLANIF, ACT.ACTIV_NOMBRE AS ACTIVIDAD, IFNULL(MET.MET_UNDXHR,0) AS META,
				IFNULL(SUM(PLANO.PLANO_HORASPROD),0) AS HORASPROD, SUM(CASE WHEN PLANO.PLANO_ASIST = 'S' THEN 1 ELSE 0 END) AS OPERADORES,
...
/*
	echo '<pre>';
	print_r($prodreal_cosecha_gloni);
	echo '</pre>';				*/	
				...
				//--$prodreal_cosecha_gloni = str_replace('.',',',$prodreal_cosecha_gloni);
	/*echo '<pre>';
	print_r($prodreal_cosecha_gloni);
	echo '</pre>';		*/			
				$hrs_cosecha_gloni = str_replace('.00','',$hrs_cosecha_gloni);
				//--$hrs_cosecha_gloni = str_replace('.',',',$hrs_cosecha_gloni);	
...
/////////////////////////////////////////////////////		
			...
			//SUM(PLANO.PLANO_HORASPROD) * SUM(PLANO.PLANO_CANT) AS PROD_REAL,
			//SUM(PLANO.PLANO_HORAS) * IFNULL(MET.MET_UNDXHR,0) AS PROD_ESPERADA,
	...
	//WHERE PLANM.ACTIV_ID IS NULL AND ACTIV.ACTIV_ID NOT IN (62, 240, 241, 128, 228, 227, 123, 432, 433) AND
	if($id_actividades == ""){
