...
ORDER BY PLAN.PLAN_DIA ASC";	
		...
		while( $rows = mysqli_fetch_assoc($resultset) ) {
			...
			if($rows[PLAN_PRODUCCION] > 0) {
				//$PLAN_PRODUCCION = round($rows[PLAN_PRODUCCION]);
				$PLAN_PRODUCCION = str_replace(',00','',str_replace(".", ",",round($rows[PLAN_PRODUCCION],2)));
				//$PLAN_PRODUCCION = str_replace('.00','',$rows[PLAN_PRODUCCION]);
				//$PLAN_PRODUCCION = str_replace(".", ",",$PLAN_PRODUCCION);
	...
	*/		
	...
	elseif(isset($_POST['change_activ'])){
		...
		elseif($operacion == 'UPDATE'){
			...
			$sql = " SELECT OPER_RUT FROM PLANIFICACION_OPER WHERE PLAN_DIA = '$dia' AND ACTIV_ID = $activ_id";				
			$resultado = mysqli_query($link,$sql);	
			$rowcount = mysqli_num_rows($resultado);	
