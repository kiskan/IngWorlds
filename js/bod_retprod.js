...
////////////TABLA SOLICITUDES/////////////////	
		...
		while(i < resp.length){
			...
			$('#productos_solicitados').append(
				'<tr>'+
					...
					'<td style="display:none" id='+cate+'>'+resp[i].CATPROD_NOMBRE+'</td>' +
	...
	$('#sap_cod').change(function(event) {
		...
		if(sap_cod == ""){
			$('#sprod_cod').val(sap_cod).trigger('change.select2');
		}
		else{
			parametros = {	
				carga_sap_id:'carga',
			...
			$.post(
				...
				function(resp){
					...
					$('#sprod_cod').val(prod_cod).trigger('change.select2');
					//$(this).unbind("change");
				},"json"
				)	
		}
	...
	$('#sprod_cod').change(function(event) {

		var prod_cod = $('#sprod_cod').val();
...
//TAB SOLICITUDES ACEPTADAS Y RECHAZADAS
	...
	function busqueda_solic_entregadas(){
		...
		//console.log(sprod_id, solicitante, per_agno, sem_num, sprod_dia, UNDVIV_ID, destinatario, prod_cod, catprod_id)
		//console.log('hola2')
		...
		while(i < resp.length){
			...
			$('#productos_aceptados').append(
				'<tr>'+
					...
					'<td style="display:none" id='+cate+' style="text-decoration-line:'+sol_aceptada+'">'+resp[i].CATPROD_NOMBRE+'</td>' +
...
////////////TABLA PRODUCTOS/////////////////	
	...
	function screar_tabla_prod(nro_solicitud){
		...
		$('#stabla_productos tbody')
			...
			$('#sprod_cod').val(obj_row.PROD_COD).trigger('change');
		...
		if(operacion == 'ACTA ENTREGA Y REBAJA STOCK'){
			...
			$.post(
				   ...
				   function(resp){
						var cod_resp = resp['cod'];
						if(cod_resp == 'ok'){
							//var url = '/proyecto/sisconvi-production2/Informes/acta_entrega.php?sprod_id='+sprod_id;
