$(document).ready(function(){
	...
	function crear_tabla( per_agno, per_mes, sem_num, plan_dia, area_id, sup_rut){
		//console.log(plan_dia)
		table = $('#datatable-scroll').DataTable( {
			ajax: "consultas/cons_cplanmaestro.php?per_agno="+per_agno+"&per_mes="+per_mes+"&sem_num="+sem_num+"&plan_dia="+plan_dia+"&area_id="+area_id+"&sup_rut="+sup_rut,
			deferRender:    true,
			scrollY:        500,
			scrollX:        true,
			scrollCollapse: true,
			...
			columns: [
				...
				{ data:'UNIDAD'},			
				{ data:'METAXHR'},
				{ data:'JORN_PLANIFICADAS'},
				{ data:'HRS_PRODUCTIVAS'},
				{ data:'OPERADORES'},
				{ data:'ACTIVIDAD_PLANIFICADA'},
				{ data:'FACTIV_NOMBRE'},
				{ data:'SFACTIV_NOMBRE'},
				{ data:'PACTIV_NOMBRE'},
				{ data:'TACTIV_NOMBRE'}				
...
//CLICK CONSULTAR
	...
	$('#btn_planmaestro').click(function(){
		table.destroy();
		...
		crear_tabla( per_agno, per_mes, sem_num, plan_dia, area_id, sup_rut);
	})
