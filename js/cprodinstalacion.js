$(document).ready(function(){
	...
	function crear_tabla( per_agno, per_mes, sem_num, plan_dia, inst_inv){
		//console.log(plan_dia)
		table = $('#datatable-scroll').DataTable( {
			ajax: "consultas/cons_cprodinstalacion.php?per_agno="+per_agno+"&per_mes="+per_mes+"&sem_num="+sem_num+"&plan_dia="+plan_dia+"&inst_inv="+inst_inv,
			deferRender:    true,
			scrollY:        500,
			scrollX:        true,
			scrollCollapse: true,
...
//CLICK CONSULTAR
	...
	$('#btn_cosecha').click(function(){
		...
		var inst_inv = $('#inst_inv').val();
		
		//console.log(per_agno +'--'+ per_mes +'--'+ sem_num +'--'+ plan_dia +'--'+ inst_inv)
				
		crear_tabla( per_agno, per_mes, sem_num, plan_dia, inst_inv);
	})
