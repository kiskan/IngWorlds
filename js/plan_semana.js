$(document).ready(function(){
	...
	function crear_tabla(num_sem, per_agno, fecha){
		...
		table = $("#datatable-responsive").DataTable({
			...
			responsive: true,
			order:[],
			ajax: "consultas/ges_crud_planxsemana.php?data="+num_sem+"&per_agno="+per_agno+"&fecha="+fecha,
			bProcessing: true,
			columns: [
				{ data:'PER_AGNO'},
				{ data:'SEM_NUM'},
	...
	$('#btn_ver_planxsemana').click(function() {
		
		var activ_id = $.trim($('#activ_id').val());
		var nom_btn = $('#btn_ver_planxsemana').html();
		
		if(activ_id == ''){
			sweet_alert('Error: Seleccione antes una actividad a planificar');
		}else{			
			
			if(nom_btn == 'Ver Disponibles'){
				$('#btn_ver_planxsemana').html('Ver Todos');
				oper_en_activ('D');
			}else{
				$('#btn_ver_planxsemana').html('Ver Disponibles');
				oper_en_activ('T');
			}			
		}
	...
	function oper_en_activ(disp){
		...
		if(a == null){
			...
			if(activ_id != ""){
				...
				parametros = {			
					change_activ:change_activ,disp:disp,h_plan_dia:h_plan_dia,h_activ_id:h_activ_split,fecha_dia:fecha_dia,sem_num:sem_num,plan_dia:plan_dia,activ_id:activ_split[0],rut_oper:rut_oper, activ_standar:activ_split[3], per_agno: per_agno
				}
				console.log(parametros)
				$.post(
...
clase_tooltip2 = ''
		if(dia != '0'){				
			...
			if(check == 'checked' && $.inArray(buscar_dia, plan_dia_text) !== -1 && dobleactiv_dia == 0){
				...
				sel_oper_dia++;					
				//dia_return = '<i class="fa fa-line-chart"></i><p id='+id_diahrdisp+' style="margin-bottom: 0px;">Dis:'+hr_dia_disp+'</p><p id='+id_diahrocup+' style="color:'+color+'; margin-bottom: 0px;">Sel:'+hr_dia_ocup+'</p>';					

				dia_return = '<i class="fa fa-user fa-lg"></i><p id='+id_diahrdisp+' style="margin-bottom: 0px;">Disp:'+hr_dia_disp+'</p><p id='+id_diahrocup+' style="color:'+color+'; margin-bottom: 0px;">Selec:'+hr_dia_ocup+'</p>';	
				...
				hr_dia_ocup = String(hr_dia_ocup).replace('.',',');
				sel_oper_dia++;					
				//dia_return = '<i class="fa fa-line-chart"></i><abbr class="tip"><span class="'+clase_tooltip+'">Area: '+area_oper+' <br>Actividad: '+activ_oper+'<br>Horas: '+horas_oper+' </span></abbr><div class="clearfix"></div><p id='+id_diahrdisp+' style="margin-bottom:0px;">Dis:'+hr_dia_disp+'</p><p id='+id_diahrocup+' style="color:'+color+'; margin-bottom: 0px;">Sel:'+hr_dia_ocup+'</p>';	

				dia_return = '<abbr class="tip"><i class="fa fa-user-plus fa-lg"></i><span class="'+clase_tooltip+'">Area: '+area_oper+' <br>Actividad: '+activ_oper+'<br>Horas: '+horas_oper+' </span></abbr><br><p id='+id_diahrdisp+' style="margin-bottom:0px;">Disp:'+hr_dia_disp+'</p><p id='+id_diahrocup+' style="color:'+color+'; margin-bottom: 0px;">Selec:'+hr_dia_ocup+'</p>';					
			}
			else{
				//dia_return = '<i class="fa fa-line-chart"></i><abbr class="tip"><span class="'+clase_tooltip+'">Area: '+area_oper+' <br>Actividad: '+activ_oper+'<br>Horas: '+horas_oper+' </span></abbr><div class="clearfix"></div><p id='+id_diahrdisp+' style="margin-bottom:0px;">Dis:'+hr_dia_disp+'</p><p id='+id_diahrocup+' style="color:'+color+'; margin-bottom: 0px;">Sel:0</p>';
				
				dia_return = '<abbr class="tip"><i class="fa fa-user-plus fa-lg"></i><span class="'+clase_tooltip+'">Area: '+area_oper+' <br>Actividad: '+activ_oper+'<br>Horas: '+horas_oper+' </span></abbr><br><p id='+id_diahrdisp+' style="margin-bottom:0px;">Disp:'+hr_dia_disp+'</p><p id='+id_diahrocup+' style="color:'+color+'; margin-bottom: 0px;">Selec:0</p>';				
			...
			dia_return = '<i class="fa fa-user fa-lg"></i><p id='+id_diahrdisp+' style="margin-bottom: 0px;">Dis:'+hr_dia_disp+'</p><p id='+id_diahrocup+' style="color:'+color+'; margin-bottom: 0px;">Sel:'+hr_dia_ocup+'</p>';
			disp_oper_dia = parseFloat(disp_oper_dia) + 1;
...
//////////////////////////////////	
	...
	function carga_operadores(resp){
		...
		var disp_oper_jueves = 0;var disp_oper_viernes = 0;var disp_oper_sabado = 0;var disp_oper_domingo = 0;
		
		var disp_jorn_lunes = 0;var disp_jorn_martes = 0;var disp_jorn_miercoles = 0;
		var disp_jorn_jueves = 0;var disp_jorn_viernes = 0;var disp_jorn_sabado = 0;var disp_jorn_domingo = 0;
		
		var sel_oper_lunes = 0;var sel_oper_martes = 0;var sel_oper_miercoles = 0;
		var sel_oper_jueves = 0;var sel_oper_viernes = 0;var sel_oper_sabado = 0;var sel_oper_domingo = 0;
		
		var sel_jorn_lunes = 0;	var sel_jorn_martes = 0;var sel_jorn_miercoles = 0;
		var sel_jorn_jueves = 0;var sel_jorn_viernes = 0;var sel_jorn_sabado = 0;var sel_jorn_domingo = 0;
		
		var sel_prod_lunes = 0;	var sel_prod_martes = 0;var sel_prod_miercoles = 0;
		var sel_prod_jueves = 0;var sel_prod_viernes = 0;var sel_prod_sabado = 0;var sel_prod_domingo = 0;
...
//DOMINGO			
			...
			dia_domingo = cargar_oper_dia(dom,dom2,"DOM",id_domhrdisp,id_domhrocup,check,plan_dia_text,disp_oper_domingo,disp_jorn_domingo,sel_jorn_domingo,sel_oper_domingo,clase_tooltip);
			dia_domingo = dia_domingo.split('***');
			domingo = dia_domingo[0];
			disp_oper_domingo = dia_domingo[1];disp_jorn_domingo = dia_domingo[2];
			sel_jorn_domingo = dia_domingo[3];sel_oper_domingo = dia_domingo[4]; 			
...
//LLENADO 1ERA LISTA			
			...
			'<tr id='+id_row+'>'+
			  ...
			  '<td class="col-md-3">'+resp[i].OPERARIO+'</td>' +
			  '<td class="col-md-1"><center>'+lunes+'</center></td>' +
			  '<td class="col-md-1"><center>'+martes+'</center></td>' +
			  '<td class="col-md-1"><center>'+miercoles+'</center></td>' +
			  '<td class="col-md-1"><center>'+jueves+'</center></td>' +
			  '<td class="col-md-1"><center>'+viernes+'</center></td>' +
			  '<td class="col-md-1"><center>'+sabado+'</center></td>' +
			  '<td class="col-md-1"><center>'+domingo+'</center></td>' +
			'</tr>');
...
//LLENADO 2DA LISTA			
		...
		$('#lista1_disp_operadores').append(
			'<tr id="disp_oper">'+
			  ...
			  '<td style="text-align:center">'+disp_oper_domingo+'</td>' +
			'</tr>');		
		...
		disp_jorn_domingo = parseFloat(String(disp_jorn_domingo).replace(',','.'));
		if(disp_jorn_domingo){
			disp_jorn_domingo = Math.round(disp_jorn_domingo*100)/100;
		}			
		...
		$('#lista2_disp_jornada').append(
			'<tr id="disp_jorn">'+
			  ...
			  '<td style="text-align:center">'+String(disp_jorn_domingo).replace('.',',')+'</td>' +
			'</tr>');
...
//OPERACION ACTUAL
		...
		sel_jorn_domingo = parseFloat(String(sel_jorn_domingo).replace(',','.'));
		if(sel_jorn_domingo){
			sel_jorn_domingo = Math.round(sel_jorn_domingo*100)/100;
		}		
		...
		$('#lista1_selec_jornada').append(
			'<tr id="selec_jorn">'+
			  ...
			  '<td style="text-align:center">'+String(sel_jorn_domingo).replace('.',',')+'</td>' +
			'</tr>');	
		...
		if(meta > 0){	
			...
			sel_jorn_domingo = parseFloat(String(sel_jorn_domingo).replace(',','.'));
			if(sel_jorn_domingo > 0){
				sel_prod_domingo = sel_jorn_domingo * meta;
				sel_prod_domingo = Math.round(sel_prod_domingo * 100)/100;
			}			
		}
		...
		$('#lista2_selec_produccion').append(
			'<tr id="selec_prod">'+
			  ...
			  '<td style="text-align:center">'+sel_prod_domingo+'</td>' +
			'</tr>');
		...
		$('#lista3_selec_operadores').append(
			'<tr id="selec_oper">'+
			  ...
			  '<td style="text-align:center">'+sel_oper_domingo+'</td>' +
			'</tr>');			
	...
	function clic_operador(e){
		...
		if(plan_dia == ''){
			...
			if($("#"+rut_oper).is(":checked")) {		
				...
				if($.inArray('DOM', plan_dia_text) !== -1 && error == 0 ){
					...
					if(disponibilidad == 0){
						...
						if(hora_disponible == '0'){
							oper_jornada(rut_oper,'domhrocup',6,disponibilidad,'SUMA');	
						}else{
				...
				if($.inArray('DOM', plan_dia_text) !== -1 ){
					domhr_disp = jornada_disponible(rut_oper,'domhrdisp');				
					oper_jornada(rut_oper,'domhrocup',6,domhr_disp,'RESTA');
				}				
				calcular_produccion();
			}
		}
	...
	$('#btn_operadores').click(function() {
		...
		if(plan_dia == ''){
			...
			if(num_operadores > total_operadores){
				...
				sum_jorn_lun = 0;sum_jorn_mar = 0;sum_jorn_mie = 0;sum_jorn_jue = 0;sum_jorn_vie = 0;sum_jorn_sab = 0;sum_jorn_dom = 0;
				deuda_oper_lun = 0;deuda_oper_mar = 0;deuda_oper_mie = 0;deuda_oper_jue = 0;deuda_oper_vie = 0;deuda_oper_sab = 0;deuda_oper_dom = 0;
				$('.check_oper').each( function() {
					...
					if(num_operadores > 0){ 
						...
						if($.inArray('DOM', plan_dia_text) !== -1 ){
							domhr_disp = jornada_disponible(this.id,'domhrdisp')
							if(domhr_disp > 0 && domhr_disp >= hora_activ){
								if(hora_activ > 0){
									domhr_disp = hora_activ
								}															
								sum_jorn_dom = sum_jorn_dom + parseFloat(domhr_disp);
								domhr_disp = String(domhr_disp).replace('.',',');
								$('#domhrocup_'+this.id).html('Selec:'+domhr_disp).css('color','green');								 
								chequear = 'S';
							}else{
								chequear = 'N';
								deuda_oper_dom++;
					...
					if(num_operadores < 0 && deuda_oper_dom > 0){
						domhr_disp = jornada_disponible(this.id,'domhrdisp')
						if(domhr_disp > 0 && domhr_disp >= hora_activ){
							if(hora_activ > 0){
								domhr_disp = hora_activ
				...
				if($.inArray('DOM', plan_dia_text) !== -1 ){
					if(meta != '' && meta > 0){
						produccion = sum_jorn_dom * meta;
						produccion = Math.round(produccion * 100)/100;
						produccion = String(produccion).replace('.',',');						
						$('#selec_prod').children('td').eq(6).text(produccion);
					}	
					sum_jorn_dom = String(sum_jorn_dom).replace('.',',');
					$('#selec_jorn').children('td').eq(6).text(sum_jorn_dom);
					$('#selec_oper').children('td').eq(6).text(num_operadores);					
				}				
			}
		}
	})
	...
	function validar_oper_disp(cant_oper){
		...
		sum_oper_lun = 0;sum_oper_mar = 0;sum_oper_mie = 0;sum_oper_jue = 0;sum_oper_vie = 0;sum_oper_sab = 0;sum_oper_dom = 0;
		$('.check_oper').each( function() {
			...
			if($.inArray('DOM', plan_dia_text) !== -1 ){
				domhr_disp = jornada_disponible(this.id,'domhrdisp')
				if(domhr_disp > 0 && domhr_disp >= hora_activ){sum_oper_dom++}				
			}			
		})
		...
		if($.inArray('DOM', plan_dia_text) !== -1 ){
			if(sum_oper_dom < cant_oper)return false;			
		}		
		return true;
	}
	...
	function validar_jorn_disp(jornada){
		...
		$('.check_oper').each( function() {
			...
			if($.inArray('DOM', plan_dia_text) !== -1 ){
				domhr_disp = jornada_disponible(this.id,'domhrdisp')
				//sum_jorn_dom = sum_jorn_dom + domhr_disp			
				if(domhr_disp > 0 && domhr_disp >= hora_activ){
					if(hora_activ > 0){
						domhr_disp = hora_activ
					}
		...
		if($.inArray('DOM', plan_dia_text) !== -1 ){
			if(sum_jorn_dom < jornada)return false;			
		}		
		return true;
	}	
		...
		if(plan_dia == ''){
			...
			if(meta == '' || meta == 0){
				...
				$('.check_oper').each( function() {
					...
					if($.inArray('DOM', plan_dia_text) !== -1 ){	
						if(sum_jorn_dom != jornada){
							domhr_disp = jornada_disponible(this.id,'domhrdisp')
							domhr_disp2 = jornada_disponible(this.id,'domhrdisp')
							if(domhr_disp > 0 && domhr_disp >= hora_activ){
								if(hora_activ > 0){
									domhr_disp = hora_activ
								}
								sum_jorn_dom2 = sum_jorn_dom;
								sum_jorn_dom = sum_jorn_dom + parseFloat(domhr_disp);
								sum_jorn_dom = Math.round(sum_jorn_dom * 100)/100;
								if(sum_jorn_dom > jornada){
									//domhr_disp = sum_jorn_dom - jornada;
									domhr_disp = domhr_disp - (sum_jorn_dom - jornada);
									domhr_disp = Math.round(domhr_disp * 100)/100;
									sum_jorn_dom = jornada;
								}
								
								if(domhr_disp2 < domhr_disp){
									sum_jorn_dom = sum_jorn_dom2;
								}else{								
									sum_oper_dom++;
									domhr_disp = String(domhr_disp).replace('.',',');
									$('#domhrocup_'+this.id).html('Selec:'+domhr_disp).css('color','green');						
				...
				if($.inArray('DOM', plan_dia_text) !== -1 ){
					...
					sum_jorn_dom = String(sum_jorn_dom).replace('.',',');
					$('#selec_jorn').children('td').eq(6).text(sum_jorn_dom);
					$('#selec_oper').children('td').eq(6).text(sum_oper_dom);					
				}				
			}
		}
	})	
