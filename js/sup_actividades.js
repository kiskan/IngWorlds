...
*/			
			...
			else if(activ_id == 882){
				$('#oper_actividad').append(
					'<tr id='+id_row+'>'+
						'<td style="text-align:center"><input type="checkbox" class="check_oper" id='+resp[i].OPER_RUT+' '+asistencia+'/></td>' +
						'<td><span id='+oper+' data-toggle="modal" data-target=".bs-example-modal-lg">'+resp[i].OPERARIO+'</span></td>' +
						'<td id='+cant+' data-dia="'+plan_dia+'" data-oper="'+resp[i].OPERARIO+'" data-toggle="modal" data-id="'+resp[i].OPER_RUT+'" data-target="#WModal_Cant_Instalacion" style="text-align:center">'+cantidad+'</td>' +
						'<td style="text-align:center; width:70px"><select id="'+hr_ini+'" style="width:65px" class="select2_single form-control" tabindex="-1">'+hora_inicial+'</select><select id="'+min_ini+'" style="width:65px" class="select2_single form-control" tabindex="-1">'+min_inicial+'</select></td>' +
						'<td style="text-align:center; width:70px"><select id="'+hr_fin+'" style="width:65px" class="select2_single form-control" tabindex="-1">'+hora_final+'</select><select id="'+min_fin+'" style="width:65px" class="select2_single form-control" tabindex="-1">'+min_final+'</select></td>' +
						'<td style="text-align:center; width:70px"><select id="'+dscto_hr+'" style="width:65px" class="select2_single form-control" tabindex="-1">'+dscto_hora+'</select><select id="'+dscto_min+'" style="width:65px" class="select2_single form-control" tabindex="-1">'+min_inicial_descto+'</select></td>' +
						'<td style="text-align:center"><span id="'+hrasig+'">'+resp[i].PLANO_HORAS+' </span></td>' +
...
*/	
	...
	$('#btn_consprod').click(function(){
        ...
        {
			oper_rut = this.id;
			//console.log('rut:'+oper_rut)
			idcant = 'cant_'+oper_rut
			cantidad= $('#'+idcant).html();
			//console.log('cantidad:'+cantidad)
			if(cantidad != ''){
				suma_cant = suma_cant + parseFloat(cantidad);
				hrprod = 'hrprod_'+oper_rut
				horas= $('#'+hrprod).html();
				//console.log('horas:'+horas)
				if(horas != ''){
					suma_prod = suma_prod + parseFloat(horas);
				}
				//console.log('suma_prod:'+suma_prod)
			}
        })
...
/*	
	$('#btn_consprod').click(function(){
        ...
        {
			rut_oper = $(this).find("tr").eq(0).html();
			console.log('rut:'+rut_oper)
			cantidad = $(this).find("td").eq(2).html();
			if(cantidad != ''){
				suma_cant = suma_cant + parseFloat(cantidad);
				//horas = $(this).find("td").eq(7).html();
				hrprod = 'hrprod_'+rut_oper
				horas= $('#'+hrprod).val();
				console.log('horas:'+horas)
				if(horas != ''){
					suma_prod = suma_prod + parseFloat(horas);
...
////////////////////////////////////////////
	...
	function crear_tabla_instalacion(){
		...
		console.log(oper_rut)
		table_inst = $("#datatable-responsiveinst").DataTable({			
			...
			ajax: "consultas/ges_crud_instalacion.php?oper_rut="+oper_rut+"&plan_dia="+plan_dia+"&activ_id="+activ_id,	
			bPaginate:true,
			bProcessing: true,
			columns: [
				{ data:'inst_id'},
		...
		$('#datatable-responsiveinst tbody')
			.on( 'click', 'tr', function () {
			if ( $(this).hasClass('selected') ) {
				$(this).removeClass('selected');
			}
			else {
				table_inst.$('tr.selected').removeClass('selected');
				$(this).addClass('selected');
			}
			
			var obj_row = table_inst.row(this).data();		
			console.log(obj_row)
			console.log(obj_row.clon_id)
			$('#inst_inv').val(obj_row.inst_inv).trigger('change');
			$('#inst_clon').val(obj_row.clon_id).trigger('change');
			$('#inst_linea').val(obj_row.inst_linea).trigger('change');
			$('#inst_tipoestaca').val(obj_row.inst_tipoestaca).trigger('change');
			$('#inst_cantidad').val(obj_row.inst_cantidad);
			
			$('#h_inst_id').val(obj_row.inst_id);			
	...
	function carga_clones(){
		...
		//console.log(data_clones)
		$('#inst_clon').select2({			
			allowClear: true,
			data:combobox,
			width: '100%',
			dropdownParent: $('#WModal_Cant_Instalacion')
		});		
			
	}	


	$('#inst_cantidad').keypress(function(e){
		if(e.which!=8 && e.which!=0 && e.which!=44 && (e.which<48 || e.which>57))return false;				
	});	

	$("#WModal_Cant_Instalacion").on('show.bs.modal', function(event){
		...
		$('#h_oper_clic').val(oper_rut);
		$('#title_instalacion').html(nom_operador)
		$('#inst_clon').select2({width: '100%'});
		
		if(typeof table_inst != 'undefined') table_inst.destroy()
		crear_tabla_instalacion();
    });	

	function validar_inst(){

		var inst_clon = $.trim($('#inst_clon').val());
		var inst_cantidad = $.trim($('#inst_cantidad').val());
	...
	$('#btn_reg_inst').click(function(){

		if (validar_inst()){	

			$('#loading_ins').show();
			
			var inst_inv = $.trim($('#inst_inv').val());
			var inst_clon = $.trim($('#inst_clon').val());
			var inst_linea = $.trim($('#inst_linea').val());
			var inst_tipoestaca = $.trim($('#inst_tipoestaca').val());
			var inst_cantidad = $.trim($('#inst_cantidad').val());
			...
			var operacion = 'REGISTRAR CANT INSTALACION';
			parametros = {			
							inst_inv:inst_inv,
							inst_clon:inst_clon,
							inst_linea:inst_linea,
							inst_tipoestaca:inst_tipoestaca,
							inst_cantidad:inst_cantidad,
			...
			$.post(
				   ...
				   parametros,			   
				   function(resp){
						ges_crud_instalacion(resp)
				   },"json"
			)
		}	
	})	
	
 	$('#btn_upd_inst').click(function(){
		if (validar_inst()){	

			$('#loading_ins').show();
			
			var inst_id = $.trim($('#h_inst_id').val());
			var inst_inv = $.trim($('#inst_inv').val());
			var inst_clon = $.trim($('#inst_clon').val());
			var inst_linea = $.trim($('#inst_linea').val());
			var inst_tipoestaca = $.trim($('#inst_tipoestaca').val());
			...
			var operacion = 'MODIFICAR CANT INSTALACION';
			parametros = {
							inst_id:inst_id,
							inst_inv:inst_inv,
							inst_clon:inst_clon,
 	...
 	$('#btn_del_inst').click(function(){
		...
		swal({   
			...
			function(){   
				...
				var operacion = 'ELIMINAR CANT INSTALACION';
				parametros = {	
								inst_id:inst_id,
	...
	function ges_crud_instalacion(resp){
		$('#loading_ins').hide();
		var cod_resp = resp['cod'];
		console.log(resp)
		if(cod_resp == 'ok'){
			...
			if(operacion == 'REGISTRAR CANT INSTALACION'){
				swal("Sisconvi-Production", "Cantidad registrada", "success");
			}else if(operacion == 'MODIFICAR CANT INSTALACION'){
				swal("Sisconvi-Production", "Cantidad modificada", "success");
			}else if(operacion == 'ELIMINAR CANT INSTALACION'){
				swal("Sisconvi-Production", "Cantidad eliminada", "success");
			}		
			
			$('#datatable-responsiveinst tbody').unbind( "click" );
			table_inst.destroy();
			crear_tabla_instalacion();
	...
	$('#btn_can_inst').click(function(){
		cancelar_inst();
	})	
	
	function cancelar_inst(){
		...
		$('#inst_cantidad').val('')
		$('#inst_inv').val('1').trigger('change');
		$('#inst_clon').val('').trigger('change');
		$('#inst_linea').val('1').trigger('change');
		$('#inst_tipoestaca').val('MACRO').trigger('change');
		$('#inst_cantidad').focus();
	}	
